...
export function MySQLNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative px-4 py-3 min-w-[240px] rounded-xl border-2 transition-all ${
      ...
      <div className="flex items-center gap-2.5 mb-1">
        <div className="p-1.5 rounded-lg bg-blue-500/10"><Database size={14} className="text-blue-400" /></div>
        <span className="text-sm font-semibold text-blue-100">MySQL</span>
      </div>
      <div className="text-[10px] text-blue-300/50 ml-9 truncate max-w-[140px]">{(config?.query as string)?.slice(0, 40) || "SELECT"}</div>
