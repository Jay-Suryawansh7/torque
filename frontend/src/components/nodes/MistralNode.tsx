...
export function MistralNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative px-4 py-3 min-w-[240px] rounded-xl border-2 transition-all ${
      ...
      <div className="flex items-center gap-2.5 mb-1">
        <div className="p-1.5 rounded-lg bg-purple-500/10"><Sparkles size={14} className="text-purple-400" /></div>
        <span className="text-sm font-semibold text-purple-100">Mistral</span>
      </div>
      <div className="text-[10px] text-purple-300/50 ml-9 truncate max-w-[140px]">{(config?.model as string) || "mistral-large"}</div>
