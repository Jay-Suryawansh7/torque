...
export function ComputerUseNode({ data, selected }: NodeProps) {
  const config = data?.config as Record<string, unknown> | undefined;
  const url = (config?.browser_url as string) || "";
  return (
    <div className={`relative px-4 py-3 min-w-[240px] rounded-xl border-2 transition-all ${
      ...
      <div className="flex items-center gap-2.5 mb-1">
        <div className="p-1.5 rounded-lg bg-rose-500/10"><Monitor size={14} className="text-rose-400" /></div>
        <span className="text-sm font-semibold text-rose-100 truncate max-w-[200px]">{(config?.label as string) || "Computer Use"}</span>
      </div>
      <div className="text-[10px] text-rose-300/50 ml-9 font-mono truncate max-w-[150px]">{url || "browser automation"}</div>
