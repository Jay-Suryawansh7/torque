...
export function ClickUpNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`relative px-4 py-3 min-w-[240px] rounded-xl border-2 transition-all ${
      ...
      <div className="flex items-center gap-2.5 mb-1">
        <div className="p-1.5 rounded-lg bg-purple-500/10"><FileText size={14} className="text-purple-400" /></div>
        <span className="text-sm font-semibold text-purple-100">ClickUp</span>
      </div>
      <div className="text-[10px] text-purple-300/50 ml-9 truncate max-w-[140px]">{(config?.destination as string) || "no list"}</div>
