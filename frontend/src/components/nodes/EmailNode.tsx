...
export function EmailNode({ data, selected }: NodeProps) {
  ...
  const dest = (config?.destination as string) || "no destination";
  return (
    <div
      className={`relative px-4 py-3 min-w-[240px] rounded-xl border-2 transition-all ${
        selected
          ...
          : "border-blue-800/30 bg-blue-500/10 hover:border-blue-700/50"
      }`}
    >
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <span className="text-sm font-semibold text-blue-100 truncate max-w-[200px]">
          {(config?.label as string) || "Email"}
        </span>
      </div>
      <div className="text-[10px] text-blue-300/50 font-mono ml-9">{dest}</div>
